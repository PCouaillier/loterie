...
class SqliteDbCreator
{
    ...
    public function createSqliteDb()
    {
        ...
        if ($this->isEmpty('User')) {
            ...
            $this->pdo->prepare('INSERT INTO `Room` (`engine`, `owner`) VALUES (?, ?)')
                ->execute(['guillaumeengine',
                    intval($this->pdo->query('SELECT id FROM `User` WHERE id='.$this->pdo->lastInsertId().';')->fetch()['id'])]);
            $this->pdo->commit();
        }
    }
    ...
    private function createSqliteSchemaIfNotExists()
    {
        ...
        $pdo->exec('CREATE TABLE IF NOT EXISTS `User` ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, `username` TEXT NOT NULL UNIQUE, `mail` TEXT NOT NULL UNIQUE, `password` TEXT NOT NULL, `canCreateRoom`	INTEGER NOT NULL DEFAULT 0);');
        $pdo->exec('CREATE TABLE IF NOT EXISTS `Room` ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, `engine` TEXT NOT NULL, `owner` INTEGER NOT NULL );');
